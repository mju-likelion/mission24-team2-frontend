...
경기남부 해커톤은 명지대학교, 을지대, 한국외대 글로벌캠퍼스. 한양대ERICA 총 네 학교가 모여 진행한 해커톤입니다.
...
우리는 안터넷 강의를 구매하기 전 해당 강사, 강의에 대한 평가를 찾아봅니다, 하지만, 리뷰들의 경우 일일이 해당 학원 사이트에서 찾아봐야 하는 불편함이 있었습니다.

이러한 불편함을 해결하기 위해 우리 SpotLight는 모든 학원들의 인터넷 강의를 모아서 제공하면 편하지 않을까? 라는 생각에서 시작됐습니다.

각 학원들의 강의를 한 사이트에 제공해주며 과목에 따라 필터링이 가능하며 쉽게 원하는 강의평을 찾을 수 있습니다.
...
```
첫 시작화면입니다. 시작 버튼을 눌러 게시물 목록을 볼 수 있습니다. 만약, 로그인이 안됐다면 로그인 화면으로 전환됩니다.
```
...
```
로그인은 서버에 저장된 회원과 정보가 일치할 경우 응답과 함께 JWT 토큰을 받습니다. JWT 토큰은 로컬스토라지에 저장, 관리됩니다.
```
...
```
모든 입력은 유효성 검증을 실행합니다. 만약 하나라도 유효성 검증에 실패할 시 가입 버튼이 비활성화 되며 각 에러에 맞는 에러 문구를 출력합니다.
```
...
```
작성된 리뷰들을 한눈에 볼 수 있는 공간입니다. 왼쪽 버튼을 통해 각 과목별로 필터링이 가능합니다.
```
...
```
글 등록의 경우 플랫폼을 누르면 해당 플럇폼의 강사들이 나오고 그 후 강사를 클릭하면 해당 강사의 강의 목록을 선택할 수 있습니다.
빠른 검색을 위해 강사, 강의의 경우 자동완성 기능을 이용했습니다.
외에도 제목, 본문을 입력할 수 있고 별점을 마우스를 통해 인터렉티브 하게 조작할 수 있습니다,
```
...
```
상세 리뷰를 확인할 수 있습니다. 닉네임의 경우 앞 두 글자만 보여주고 그 이후는 '*'을 통해 익명성을 보장했습니다.
```
